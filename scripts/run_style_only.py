...
def main():
    try:
        ...
        # crude frame extraction via moviepy if available
        try:
            ...
            print('Could not extract frames via moviepy; expecting frames to exist in outputs/frame_*.png')
            frames = []
            for i in range(10):
                p = f'outputs/frame_{i:03d}.png'
                if os.path.exists(p):
        ...
        try:
            ...
            with open('outputs/progress_style.json', 'w', encoding='utf-8') as pf:
                json.dump({'pct': 10, 'stage': 'starting'}, pf)
        except Exception:
            pass
        style_tl = build_style_timeline(semantic, fps=fps, duration_sec=duration)
        beat = []
        try:
